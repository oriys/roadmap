    ...
    weeks: [
      {
        ...
        lessons: [
          ...
          {
            id: "bp-w3-3",
            title: "CQRS、分库分表与 Redis 热点治理",
            detail: "通过读写分离、水平分片与热点 Key 治理应对大规模数据访问性能挑战。",
            resources: [
              { title: "CQRS - Martin Fowler", url: "https://martinfowler.com/bliki/CQRS.html" },
              { title: "ShardingSphere 官方文档", url: "https://shardingsphere.apache.org/document/current/en/overview/" },
              { title: "Redis Big Key 问题与解决", url: "https://redis.io/docs/latest/develop/use/keyspace/" },
            ],
          },
        ],
      },
        ...
        lessons: [
          ...
          },
          {
            ...
            resources: [
              ...
              { title: "Nginx SSL Best Practices", url: "https://nginx.org/en/docs/http/configuring_https_servers.html" },
            ],
          },
        ],
      },
        ...
        lessons: [
          ...
          },
          {
            ...
            resources: [
              ...
              { title: "Prometheus Adapter", url: "https://github.com/kubernetes-sigs/prometheus-adapter" },
            ],
          },
        ],
      },
...
export const backendPerformanceExamQuestions: QuizQuestion[] = [
  ...
  {
    id: "bp-q6",
    question: "使用火焰图定位性能瓶颈时，应重点关注什么？",
    options: [
      "函数名称的字母排序",
      "调用栈中宽度最大的帧——它代表 on-CPU 占比最高的热点",
      "最深层的调用栈，因为深度决定耗时",
      "只看叶子节点的平均耗时",
    ],
    answer: 1,
    rationale: "火焰图中帧的宽度对应采样占比，最宽的帧表示 CPU 时间占比最高的热点路径。",
  },
  {
    id: "bp-q7",
    question: "在高并发流式处理中，背压（Backpressure）机制的核心作用是什么？",
    options: [
      "让消费者丢弃所有处理不了的消息",
      "通过反馈信号减缓生产者速率，防止中间缓冲区溢出",
      "无限扩大缓冲区以容纳所有消息",
      "将所有请求转为同步阻塞调用",
    ],
    answer: 1,
    rationale: "背压通过向上游传递压力信号来控速，防止缓冲区无限增长导致 OOM 或延迟飙升。",
  },
  {
    id: "bp-q8",
    question: "相比 JSON，选择 Protocol Buffers 作为微服务间序列化格式的主要性能优势是什么？",
    options: [
      "人类可读性更好",
      "二进制编码体积更小、序列化/反序列化速度更快",
      "不需要定义 Schema",
      "自带压缩算法无需额外处理",
    ],
    answer: 1,
    rationale: "Protobuf 采用紧凑的二进制编码，比 JSON 体积小 3-10 倍，解析速度快数倍。",
  },
  {
    id: "bp-q9",
    question: "Kubernetes HPA 在应对突发流量时存在扩容延迟，以下哪种做法最有效？",
    options: [
      "将 HPA 指标采集间隔设为 1 秒",
      "结合预测性伸缩提前扩容，并用预热探针确保新 Pod 就绪后再接流量",
      "去掉就绪探针让 Pod 更快上线",
      "把所有 Pod 的 CPU Request 设为 0 以加速调度",
    ],
    answer: 1,
    rationale: "扩容延迟 = 指标采集 + 调度 + 启动 + 预热；预测性伸缩提前扩容并用就绪探针保障质量。",
  },
  {
    id: "bp-q10",
    question: "Linux 零拷贝技术（如 sendfile）能显著提升文件传输性能的根本原因是什么？",
    options: [
      "它使用更快的磁盘驱动程序",
      "它减少了内核态与用户态之间的数据拷贝和上下文切换次数",
      "它绕过了 TCP 协议直接发送原始数据",
      "它将文件全部加载到 GPU 显存中处理",
    ],
    answer: 1,
    rationale: "传统 I/O 需要 4 次拷贝和多次上下文切换；sendfile 在内核态直接完成传输，减少到最少 2 次拷贝。",
  },
  {
    id: "bp-q11",
    question: "Java 应用出现 P999 延迟毛刺，GC 日志显示频繁的 Full GC 停顿，以下哪项措施最合适？",
    options: [
      "关闭 GC 以避免停顿",
      "无限增大堆内存使 GC 永远不触发",
      "评估切换到低延迟收集器（如 ZGC），并调优堆大小与对象分配模式",
      "将所有对象改为 static 避免被回收",
    ],
    answer: 2,
    rationale: "ZGC 将暂停控制在 10ms 内且与堆大小无关；同时应优化分配模式减少 GC 压力。",
  },
  {
    id: "bp-q12",
    question: "关于数据库连接池大小的设定，以下哪项描述最准确？",
    options: [
      "连接数越多性能越好，应设为数百",
      "最佳连接数通常远小于预期，可参考公式 (CPU 核心数 × 2) + 磁盘数",
      "连接池应禁用空闲超时，让连接永不关闭",
      "每个请求独占一个新连接比连接池更高效",
    ],
    answer: 1,
    rationale: "过多连接增加上下文切换和锁竞争，HikariCP 建议基于 CPU 和磁盘数计算最佳值。",
  },
  {
    id: "bp-q13",
    question: "在 CQRS 架构中，读模型使用 Elasticsearch，写模型使用 MySQL。当用户刚写入数据后立即查询却看不到结果，最可能的原因和正确处理方式是什么？",
    options: [
      "Elasticsearch 出了 bug，应换用 MySQL 全文搜索",
      "这是最终一致性的正常表现，应在 UI 层做乐观更新或引导用户预期",
      "应该让写操作同步阻塞直到 ES 索引完成",
      "去掉读写分离，统一用一个数据源",
    ],
    answer: 1,
    rationale: "CQRS 的读模型是异步构建的，存在同步延迟；应通过 UI 层乐观更新或适当提示来处理。",
  },
  {
    id: "bp-q14",
    question: "在高并发 HTTPS 服务中，以下哪项最能降低 TLS 握手对延迟的影响？",
    options: [
      "降级到 TLS 1.0 以减少握手复杂度",
      "启用 TLS 1.3 + Session Ticket 复用 + OCSP Stapling",
      "关闭证书验证以跳过握手",
      "把所有接口改为 HTTP 明文传输",
    ],
    answer: 1,
    rationale: "TLS 1.3 将握手减至 1-RTT，Session Ticket 实现 0-RTT 恢复，OCSP Stapling 避免客户端额外查询。",
  },
  {
    id: "bp-q15",
    question: "FinOps 实践中，衡量后端性能优化 ROI 最有效的指标是什么？",
    options: [
      "代码行数减少量",
      "单次请求成本（Cost per Request）及其随优化的变化趋势",
      "服务器数量的绝对值",
      "开发者加班时间的减少",
    ],
    answer: 1,
    rationale: "Cost per Request 将云成本与业务量关联，能直观反映优化带来的单位经济效益。",
  },
  {
    id: "bp-q16",
    question: "eBPF 相比传统内核模块在性能观测方面的核心优势是什么？",
    options: [
      "它只能在用户态运行，无法访问内核数据",
      "它需要重新编译内核才能使用",
      "它在内核中安全沙箱运行，无需修改内核或重启即可动态追踪",
      "它只能追踪网络流量，不能观测系统调用",
    ],
    answer: 2,
    rationale: "eBPF 程序在内核验证器保障下安全运行，可以动态挂载到几乎任何内核探测点而无需重启。",
  },
  {
    id: "bp-q17",
    question: "LSM-tree 存储引擎（如 RocksDB）相比 B-tree 的主要性能权衡是什么？",
    options: [
      "LSM-tree 读写性能都优于 B-tree",
      "LSM-tree 写入吞吐高但存在读放大和空间放大，需要通过 Compaction 优化",
      "LSM-tree 不支持持久化存储",
      "B-tree 只能用于内存数据库",
    ],
    answer: 1,
    rationale: "LSM-tree 将随机写转为顺序写获得高写入吞吐，但读取需合并多层数据，且 Compaction 带来写放大。",
  },
  {
    id: "bp-q18",
    question: "混沌工程实验中，「稳态假设」的作用是什么？",
    options: [
      "确保实验永远不会影响系统",
      "定义系统正常行为的可观测指标，作为实验前后的对比基准",
      "让混沌实验只在开发环境运行",
      "保证注入的故障一定会被自动修复",
    ],
    answer: 1,
    rationale: "稳态假设用可量化的指标描述系统正常行为，实验通过对比注入前后的指标偏移来验证韧性。",
  },
  {
    id: "bp-q19",
    question: "在全球化部署中，使用 Anycast 进行负载均衡的核心优势是什么？",
    options: [
      "它能自动翻译不同语言的请求",
      "多个地理位置的服务器共享同一 IP，BGP 路由将用户导向最近节点",
      "它绕过 DNS 解析直接建立连接",
      "它只适用于 UDP 协议，不能用于 TCP",
    ],
    answer: 1,
    rationale: "Anycast 通过 BGP 路由广播同一 IP，让用户自动连接到网络路径最短的节点，降低延迟。",
  },
  {
    id: "bp-q20",
    question: "使用通用可扩展性定律（USL）进行容量规划时，该模型考虑了哪两个限制因素？",
    options: [
      "网络带宽和磁盘 IOPS",
      "竞争（Contention）和一致性（Coherency），分别导致线性偏离和回退",
      "CPU 频率和内存容量",
      "代码行数和部署频率",
    ],
    answer: 1,
    rationale: "USL 在 Amdahl 定律基础上增加了一致性惩罚项，竞争导致吞吐亚线性增长，一致性导致超过拐点后吞吐反降。",
  },
]
