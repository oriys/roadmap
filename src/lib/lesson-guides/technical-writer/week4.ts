...
export const week4Guides: Record<string, LessonGuide> = {
    ...
    "tw-w4-4": {
        lessonId: "tw-w4-4",
        background: [
            "【CLI 文档核心要素】命令用途、参数说明（短/长选项）、示例、返回值/退出码、错误信息——这些是用户使用 CLI 工具的基本需要。",
            "【GNU 命令行标准】短选项用单破折号（-v）、长选项用双破折号（--verbose）、选项可以有参数、选项顺序通常不敏感。这是 Unix/Linux 命令的事实标准。",
            "【Cobra 框架能力】Go 语言 CLI 框架，被 173,000+ 项目使用（Kubernetes、Docker、GitHub CLI）。支持无限嵌套命令、自动帮助生成、shell 补全、man page 和 Markdown 文档自动生成。",
            "【Click 框架特点】Python CLI 框架，'创建美观的命令行界面，代码量最少'。支持命令嵌套、自动帮助生成、动态加载子命令，帮助信息由装饰器和 docstring 自动提取。",
            "【文档层次结构】快速参考（常用命令速查）、完整参考（所有命令和选项）、教程（常见任务的步骤）——不同用户需要不同层次的文档。"
        ],
        keyDifficulties: [
            "【命令层级组织】子命令的嵌套（如 git remote add）、全局选项 vs 子命令选项、如何让用户快速找到所需命令——需要清晰的导航结构。",
            "【参数格式说明】必需 vs 可选（用 [] 表示可选）、多值参数、互斥选项、默认值——每个参数的行为都要说清楚。",
            "【退出码文档化】不同退出码代表什么（0 成功、非 0 失败的具体类型）、用户如何在脚本中使用退出码——这对自动化至关重要。",
            "【帮助信息设计】--help 面向正在工作中的用户，需要快速查阅。应简洁列出关键信息，详细说明链接到在线文档。",
            "【环境变量与配置】CLI 支持的环境变量、配置文件位置、优先级规则（命令行 > 环境变量 > 配置文件）——多种配置方式的交互需要清晰说明。"
        ],
        handsOnPath: [
            "为一个 CLI 工具写命令参考：包含所有命令、选项、参数、默认值，按逻辑分组（如管理命令、查询命令）。",
            "为最常用的 5 个命令写详细示例：展示输入、输出、常见选项组合，覆盖典型使用场景。",
            "记录退出码：列出所有可能的退出码和含义（如 0=成功、1=一般错误、2=参数错误），说明如何在脚本中检查。",
            "写一份 CLI 快速入门：5 分钟内完成安装和第一次使用，覆盖最核心的功能，让用户快速上手。",
            "设计帮助信息结构：Usage（用法）、Description（描述）、Options（选项列表），确保在终端中直接可读。"
        ],
        selfCheck: [
            "【可发现性】用户能否从文档快速找到所需命令？命令是否按逻辑分组？是否有索引或搜索？",
            "【参数完整性】每个命令是否说明了：用途、必需参数、可选参数、默认值、参数格式？",
            "【示例充分性】是否有足够的示例？示例是否覆盖了常见使用场景和选项组合？",
            "【退出码文档化】退出码是否有文档？用户能否在脚本中正确使用退出码判断执行结果？",
            "【配置优先级】环境变量和配置文件是否有说明？优先级规则是否清晰？"
        ],
        extensions: [
            "【GNU 标准】学习 GNU 命令行标准：https://www.gnu.org/prep/standards/ —— 理解 POSIX 兼容的选项和参数约定。",
            "【Cobra 深入】Cobra 框架指南：https://cobra.dev/ —— 学习如何利用框架自动生成文档和 shell 补全。",
            "【Click 文档】Click 框架参考：https://click.palletsprojects.com/ —— 了解 Python CLI 的最佳实践和帮助信息生成。",
            "【优秀案例】研究 kubectl、docker、git 的 CLI 文档组织方式——它们是行业标杆。"
        ],
        sourceUrls: [
