...
export const week8Guides: Record<string, LessonGuide> = {
    "tw-w8-1": {
        lessonId: "tw-w8-1",
        background: [
            "作品集是技术写作面试中最有效的材料——展示你实际完成的工作，比任何描述都更有说服力。作品集应展示你'为之感到骄傲'或'展示特定技能'的作品。",
            "针对不同职位定制作品集：编辑岗位需要编辑前后对比，API 文档岗位要展示 HTTP 和 OpenAPI 知识，开发者文档岗位要展示代码示例能力。",
            "严格禁止包含未获授权的内容：'当你为他人工作时，他们拥有你创作的内容，而非你'。需获得书面明确许可，且不能包含密码、商业机密或敏感信息。",
            "托管方式选择：云存储（Google Drive、Dropbox）、自建网站、WordPress、静态网站生成器（Hugo、VitePress、MkDocs）。'有作品集比打造完美作品集更重要'。"
        ],
        keyDifficulties: [
            "无权使用真实工作时的替代方案：记录常用工具、编写游戏攻略、为开源项目撰写文档。这展现了'你擅长什么样的写作，以及你想向哪个方向发展'。",
            "作品集的范围选择：不需要完整的文档站点，可以是单个高质量的 README、API 参考页面、或教程。质量优于数量。",
            "展示过程而非只有结果：如果可能，展示你的设计决策、修订过程、用户反馈如何影响了最终版本。",
            "技术栈的选择：作为技术写作者，使用 SSG 构建作品集本身就是展示技能的机会。但不要让技术实现阻碍内容创作。"
        ],
        handsOnPath: [
            "选择一个开源项目或个人项目，用 Diátaxis 框架规划文档结构：确定需要哪些 Tutorial、How-to、Reference、Explanation。",
            "创建文档站点：使用 VitePress/MkDocs/Docusaurus，配置导航和主题，部署到 GitHub Pages 或 Netlify。",
            "撰写关键页面：至少包含一份 Quickstart、一份 How-to、一份 API Reference。确保代码示例可运行。",
            "添加质量保障：配置 markdownlint、链接检查，设置 CI 自动检查，展示你对 Docs-as-Code 的理解。"
        ],
        selfCheck: [
            "你的作品集是否可在线访问？招聘方能否通过 URL 直接查看？",
            "你是否展示了多种文档类型（教程、指南、参考）？是否覆盖了目标职位需要的技能？",
            "你的作品是否有变更记录（Git history）？能否展示你的修订过程？",
            "代码示例是否可运行？你是否实际测试过？",
            "你是否获得了所有作品的使用授权？是否避免了敏感信息？"
        ],
        extensions: [
            "学习 Write the Docs 作品集指南：https://www.writethedocs.org/hiring-guide/portfolios.html",
            "研究优秀的开源文档站点：Stripe、Twilio、Tailwind CSS 的文档设计。",
            "为你的作品集添加'设计说明'：解释你的信息架构决策、受众分析、迭代过程。",
            "向 Write the Docs 社区寻求反馈：Slack 频道或本地 Meetup。"
        ],
        sourceUrls: [
