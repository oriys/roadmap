...
export const customLessonQuizzes: Record<string, QuizQuestion[]> = {
  ...
  "w1-1": [
    {
      id: "w1-1-q1",
      question: "man7 列出的 namespace 类型正确的是？",
      options: [
        "pid/uts/ipc/mnt/net/user/cgroup（部分内核还包含 time）",
        "仅有 SELinux/AppArmor 两种",
        "只有 network 一种",
        "需要安装第三方内核模块才有 namespace",
      ],
      answer: 0,
      rationale: "namespaces.7 说明内核原生提供 PID/UTS/IPC/Mount/Network/User/Cgroup 等隔离。",
    },
    {
      id: "w1-1-q2",
      question: "一个 namespace 在什么情况下会被内核回收？",
      options: [
        "最后一个关联的进程/文件描述符消失时即被销毁",
        "必须手工 umount 才能销毁",
        "会一直存在直至重启",
        "需要删除 /proc/<pid>/ns 下的文件",
      ],
      answer: 0,
      rationale: "文档指出 namespace 由引用计数管理，最后引用消失后内核释放。",
    },
    {
      id: "w1-1-q3",
      question: "进入新的 user namespace 后的 root 拥有哪些特性？",
      options: [
        "在该 user namespace 内拥有一套独立 capabilities，并非宿主机全能 root",
        "自动成为宿主机 root",
        "失去所有权限，无法挂载或设置 hostname",
        "只能访问 /proc，不可再创建其它 namespace",
      ],
      answer: 0,
      rationale: "userns 会重新映射 UID/GID，root 权限被限制在当前命名空间。",
    },
    {
      id: "w1-1-q4",
      question: "在新的 PID namespace 中，/proc 显示的内容是？",
      options: ["仅显示该 PID namespace 的进程视图并重新编号", "仍显示宿主机全部进程", "完全空白", "只显示内核线程"],
      answer: 0,
      rationale: "PID namespace 让 /proc 呈现局部 PID 树，外部 PID 会被重新映射。",
    },
    {
      id: "w1-1-q5",
      question: "NET namespace 默认联网能力如何？",
      options: [
        "默认没有外部连通性，需要 veth/bridge/路由等手工连接",
        "自动继承宿主机所有网卡与 IP",
        "自动阻断 loopback",
        "必须重启主机才能联网",
      ],
      answer: 0,
      rationale: "文档强调 NET namespace 需显式连接到其它命名空间或物理网卡。",
    },
    {
      id: "w1-1-q6",
      question: "Mount namespace 的作用是？",
      options: [
        "提供独立的挂载视图，mount/umount 不影响其他命名空间",
        "提高磁盘 IOPS",
        "自动开启只读根文件系统",
        "阻断所有 /proc 访问",
      ],
      answer: 0,
      rationale: "CLONE_NEWNS 创建的挂载命名空间使挂载操作局部化。",
    },
    {
      id: "w1-1-q7",
      question: "使用 setns 进入现有 namespace 需要的权限是？",
      options: ["目标 user namespace 中的 CAP_SYS_ADMIN", "无权限要求，任意用户可进入", "CAP_NET_RAW 即可", "必须重启机器"],
      answer: 0,
      rationale: "man setns 指出加入大多数字命名空间需要在目标 userns 内具备 CAP_SYS_ADMIN。",
    },
    {
      id: "w1-1-q8",
      question: "Kubernetes 中设置 hostPID/hostNetwork=true 的效果是？",
      options: [
        "容器与宿主共享对应命名空间，进程或网络完全暴露",
        "仅提升性能，不影响隔离",
        "自动禁止容器访问宿主机",
        "让 Pod 运行在只读根文件系统",
      ],
      answer: 0,
      rationale: "host* 选项会打破 namespace 隔离，直接共享宿主视图。",
    },
    {
      id: "w1-1-q9",
      question: "/proc/<pid>/ns 目录下的符号链接代表什么？",
      options: ["指向 namespace 的内核对象，可用于 setns/bind-mount 保持引用", "普通文件，可随意删除", "配置文件，修改即可更换 namespace", "仅供调试，对命名空间无影响"],
      answer: 0,
      rationale: "man namespaces 说明这些链接用于标识 namespace inode，可被绑定保证存活。",
    },
    {
      id: "w1-1-q10",
      question: "执行 unshare -Ur 的典型结果是？",
      options: [
        "创建新的 user+uts namespace，并将当前用户映射为新 namespace 的 root",
        "直接切换到宿主 root",
        "删除当前 user namespace",
        "只能在容器内执行，宿主不可用",
      ],
      answer: 0,
      rationale: "选项 -U 创建 userns，-r 将调用者映射为 root，常用于无特权挂载等操作。",
    },
  ],
  // Week 16
  "w16-1": [
  ...
  "w1-2": [
    {
      id: "w1-2-q1",
      question: "cgroup v2 的核心设计是？",
      options: [
        "单一层级统一所有控制器，通过 cgroup.subtree_control 启用",
        "为每个控制器单独建树",
        "只有容器才能使用",
        "仅限 CPU 和内存，不含 IO/进程数",
      ],
      answer: 0,
      rationale: "内核文档指出 v2 采用统一层级，子树按需开启控制器。",
    },
    {
      id: "w1-2-q2",
      question: "cpu.max 文件的含义是？",
      options: [
        "设置 <quota> <period>，超出额度时 CFS 触发节流",
        "表示 CPU 核心总数",
        "控制平均负载值",
        "限定调度优先级",
      ],
      answer: 0,
      rationale: "cgroup v2 使用 cpu.max 来定义带宽限制，单位为微秒。",
    },
    {
      id: "w1-2-q3",
      question: "cpu.weight 在 cgroup v2 中的作用？",
      options: [
        "在同级之间按权重分配 CPU（1-10000，默认 100）",
        "限制最大线程数",
        "开启实时调度",
        "决定调度周期长度",
      ],
      answer: 0,
      rationale: "文档说明 cpu.weight 替代 v1 的 cpu.shares，用于相对权重分配。",
    },
    {
      id: "w1-2-q4",
      question: "memory.high 与 memory.max 的区别是？",
      options: [
        "high 触发渐进式回收/节流，max 是硬性上限，超出会 OOM",
        "两者完全等价",
        "high 只能限制文件缓存，max 限制匿名内存",
        "max 仅影响 swap",
      ],
      answer: 0,
      rationale: "v2 内存文档指出 memory.high 是软限制，memory.max 是硬限制。",
    },
    {
      id: "w1-2-q5",
      question: "如何在 cgroup v2 禁用 swap？",
      options: ["将 memory.swap.max 设为 0", "删除 memory.current", "设置 memory.high 为 0", "无法禁止"],
      answer: 0,
      rationale: "文档给出的例子：memory.swap.max=0 可禁止使用 swap。",
    },
    {
      id: "w1-2-q6",
      question: "pids.max 的作用是？",
      options: [
        "限制 cgroup 内可创建的进程/线程数，超出 fork 将失败",
        "限制最大文件描述符",
        "控制最大 CPU 使用率",
        "只对内核线程生效",
      ],
      answer: 0,
      rationale: "pids 控制器阻止 fork/clone，避免 fork 炸弹。",
    },
    {
      id: "w1-2-q7",
      question: "开启控制器后“no internal processes”规则意味着？",
      options: [
        "父级拥有子 cgroup 时不应再直接放置进程",
        "父级必须同时运行一个进程",
        "只能在根 cgroup 放进程",
        "与 v1 相同，无特别规则",
      ],
      answer: 0,
      rationale: "v2 规定启用控制器的节点不能同时直接包含进程和子层级。",
    },
    {
      id: "w1-2-q8",
      question: "cgroup.controllers 文件的用途是？",
      options: [
        "列出当前层级可用的控制器，写入 subtree_control 才能下沉",
        "保存运行进程列表",
        "配置权限掩码",
        "记录统计指标",
      ],
      answer: 0,
      rationale: "需先在 subtree_control 里启用 controllers，来源 cgroup v2 文档。",
    },
    {
      id: "w1-2-q9",
      question: "cgroup v2 挂载点通常位于？",
      options: ["/sys/fs/cgroup", "/var/lib/cgroup", "/proc/cgroup2", "/opt/cgroup"],
      answer: 0,
      rationale: "默认统一层级挂载在 /sys/fs/cgroup。",
    },
    {
      id: "w1-2-q10",
      question: "限制块设备 IO 吞吐通常使用哪个接口？",
      options: ["io.max（按设备 major:minor 设定带宽/IOPS）", "memory.max", "cpu.weight", "pids.max"],
      answer: 0,
      rationale: "v2 的 io 控制器使用 io.max/ io.weight 来限制吞吐或分配权重。",
    },
  ],
  "w1-3": [
    {
      id: "w1-3-q1",
      question: "挂载 overlay 时 lowerdir/upperdir/workdir/merged 的角色分别是？",
      options: [
      ...
      answer: 0,
      rationale: "overlayfs 文档描述四个目录的职责与常见挂载示例。",
    },
    {
      id: "w1-3-q2",
      question: "upperdir 与 workdir 有什么要求？",
      options: [
        "必须位于同一文件系统且为空目录",
        "可以在不同磁盘上",
        "允许与 lowerdir 相同",
      ...
      answer: 0,
      rationale: "内核文档强调 upper 与 work 需同一 fs 并预先创建空目录。",
    },
    {
      id: "w1-3-q3",
      question: "首次对 lower 层文件进行写操作会发生什么？",
      options: ["触发 copy-up，将文件复制到 upper 再修改", "直接在 lower 原地写入", "写操作被拒绝", "自动创建新 lower 层"],
      answer: 0,
      rationale: "OverlayFS 通过写时复制保护下层只读内容。",
    },
    {
      id: "w1-3-q4",
      question: "删除 lowerdir 中的文件在 merged 视图中的表现是？",
      options: [
        "在 upper 创建 whiteout/opaque 条目标记删除，从而隐藏 lower 中的同名文件",
        "直接删除 lower 原文件",
        "不会影响 merged 视图",
        "需要手动清理 workdir",
      ],
      answer: 0,
      rationale: "文档指出删除通过 whiteout 覆盖，使文件在合并视图中消失。",
    },
    {
      id: "w1-3-q5",
      question: "当指定多个 lowerdir（lowerdir=A:B:C）时，查找顺序是？",
      options: [
        "从左到右，最左侧优先",
        "从右到左，最右优先",
        "随机选择",
      ...
      answer: 0,
      rationale: "内核文档规定第一个 lower 拥有最高优先级，后续用于补充。",
    },
    {
      id: "w1-3-q6",
      question: "仅修改文件元数据（如 chmod/chown）会触发什么行为？",
      options: ["同样会触发 copy-up，再修改 upper 中的元数据", "不会触发 copy-up", "直接修改 lower", "会清空文件内容"],
      answer: 0,
      rationale: "任何需要写入的操作（包含元数据）都会复制到 upper 以保持下层只读。",
    },
    {
      id: "w1-3-q7",
      question: "lower 层在 overlay 挂载期间的访问权限是？",
      options: ["保持只读，不会被直接修改", "可读写，与 upper 相同", "自动变为不可见", "需要 root 手工 remount 才能访问"],
      answer: 0,
      rationale: "OverlayFS 只读取 lower，不会修改 lower 内容。",
    },
    {
      id: "w1-3-q8",
      question: "workdir 的用途是？",
      options: [
        "为写时复制和 rename 等操作提供临时空间",
        "保存日志",
        "存放 lower 的备份",
        "挂载完成后可以删除",
      ],
      answer: 0,
      rationale: "文档说明 workdir 是内部使用的临时目录，不可省略也不应删除。",
    },
    {
      id: "w1-3-q9",
      question: "如果 upper 中目录被标记为 opaque，会导致什么结果？",
      options: [
        "该目录下的 lower 内容在合并视图中被全部隐藏",
        "upper 无法再写入",
        "lower 自动同步到 upper",
        "merged 视图崩溃",
      ],
      answer: 0,
      rationale: "opaque xattr 告诉 overlay 忽略 lower 的同名目录内容。",
    },
    {
      id: "w1-3-q10",
      question: "Docker 的 overlay2 驱动叠加多层镜像时的特点是？",
      options: [
        "利用多 lowerdir 堆叠镜像层，upper 作为容器可写层",
        "每层都是完整拷贝，无共享",
        "只能支持单层镜像",
        "必须使用 Btrfs 才能运行",
      ],
      answer: 0,
      rationale: "官方文档描述 overlay2 通过多 lowerdir 复用镜像层，upper 记录容器写入。",
    },
  ],
  "w1-4": [
    {
      id: "w1-4-q1",
      question: "默认 bridge 网络下，容器如何与宿主机通信？",
      options: [
        "通过 veth pair 连接到 docker0 网桥，路由到宿主",
        "直接共享宿主机网卡",
        "需要额外安装代理",
        "只能通过主机端口映射",
      ],
      answer: 0,
      rationale: "Docker Bridge 文档说明每个容器有一端 veth 插入 docker0。",
    },
    {
      id: "w1-4-q2",
      question: "容器暴露服务给外部访问的典型方式是？",
      options: [
        "使用 -p/-P 进行 DNAT 端口映射",
        "默认自动暴露所有端口",
        "修改 /etc/hosts 即可",
      ...
      answer: 0,
      rationale: "官方教程指出端口映射会在 nat 表 DOCKER 链中做 DNAT。",
    },
    {
      id: "w1-4-q3",
      question: "未指定 -p 时容器的可见性是？",
      options: [
        "宿主机可通过容器 IP 访问，外部主机无法直接访问",
        "外部主机也可直接访问",
        "容器无法访问宿主机",
        "容器自动断网",
      ],
      answer: 0,
      rationale: "bridge 网络提供本机可达的私有子网，未映射端口对外不可见。",
    },
    {
      id: "w1-4-q4",
      question: "自定义 bridge 网络相较默认 docker0 的优势是？",
      options: [
        "内置 DNS 可用容器名互访，隔离不同网络的广播域",
        "性能更差且无 DNS",
        "只能用于 Swarm",
        "必须绑定固定 IP 无法自动分配",
      ],
      answer: 0,
      rationale: "官方文档：user-defined bridge 支持 DNS 名称解析与更好的隔离。",
    },
    {
      id: "w1-4-q5",
      question: "使用 --network host 的效果是？",
      options: [
        "容器与宿主共享网络命名空间，不再通过 NAT/bridge",
        "自动获得独立的私有网段",
        "禁用 DNS",
        "强制端口映射",
      ],
      answer: 0,
      rationale: "host 模式直接共享宿主网络栈，端口冲突需自行避免。",
    },
    {
      id: "w1-4-q6",
      question: "端口映射在 iptables 中通常体现为？",
      options: [
        "nat 表 PREROUTING/OUTPUT 跳转 DOCKER 链做 DNAT，POSTROUTING 做 SNAT",
        "filter 表 INPUT 链直接 ACCEPT",
        "mangle 表修改 TOS",
        "仅在 raw 表",
      ],
      answer: 0,
      rationale: "Docker 文档展示了 DOCKER 链中的 DNAT/MASQUERADE 规则。",
    },
    {
      id: "w1-4-q7",
      question: "容器访问互联网时的默认出站路径是？",
      options: [
        "经 docker0 路由到宿主，再由 MASQUERADE 转换为宿主 IP 出口",
        "直接使用容器 IP 出口，无需 NAT",
        "必须配置 HTTP 代理",
        "只能走 VPN",
      ],
      answer: 0,
      rationale: "bridge 网络通过 SNAT 让私有地址访问外网。",
    },
    {
      id: "w1-4-q8",
      question: "使用 docker network connect 对运行中容器的影响是？",
      options: [
        "容器可附加到新的网络并获取额外 IP，不会中断当前进程",
        "必须重启容器",
        "会删除旧网络",
        "只能用于停止的容器",
      ],
      answer: 0,
      rationale: "官方示例展示可在运行时连接多个网络实现多网卡效果。",
    },
    {
      id: "w1-4-q9",
      question: "veth pair 在宿主机上的表现是？",
      options: [
        "成对出现，一端放入容器 namespace，另一端留在宿主并接入 bridge",
        "单端设备，只有容器能看到",
        "需要驱动模块手工编译",
        "只在 overlay 网络存在",
      ],
      answer: 0,
      rationale: "内核 veth 文档解释虚拟网卡成对出现，可跨 namespace 连接。",
    },
    {
      id: "w1-4-q10",
      question: "要抓取容器流量，常见的抓包位置是？",
      options: ["宿主机 veth 另一端或 docker0 上抓包", "只能在物理网卡", "必须在容器内安装驱动", "无法抓取"],
      answer: 0,
      rationale: "桥接模式下，容器流量经过 veth 与 docker0，便于抓包排查。",
    },
  ],
  // Week 2
  "w2-1": [
